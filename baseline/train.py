...
def generate_log_images(inputs_t, true_masks_t, masks_pred_softmax_t):
    ...
    pad_size = 5
    images_batch = (torch.ones((bs, 3, h*3+pad_size*2, w*3+pad_size*2)) * 255.).to(device=device, dtype=torch.uint8)
    ...
    images_batch[:, 1, h*2+pad_size*2:, :w] = true_masks[:, 2, :, :]
    
    images_batch[:, :, :h, w+pad_size:w*2+pad_size] = 0
    images_batch[:, 2, :h, w+pad_size:w*2+pad_size] = true_masks[:, 3, :, :]
    
    images_batch[:, :, h+pad_size:h*2+pad_size:, w+pad_size:w*2+pad_size] = true_masks[:, 4, :, :][:, None, :, :]
    
    images_batch[:, :, h*2+pad_size*2:, w+pad_size:w*2+pad_size] = 0
    images_batch[:, 0, h*2+pad_size*2:, w+pad_size:w*2+pad_size] = masks_pred_softmax[:, 1, :, :]
    
    images_batch[:, :, :h, w*2+pad_size*2:] = 0
    images_batch[:, 1, :h, w*2+pad_size*2:] = masks_pred_softmax[:, 2, :, :]
    
    images_batch[:, :, h+pad_size:h*2+pad_size, w*2+pad_size*2:] = 0
    images_batch[:, 2, h+pad_size:h*2+pad_size, w*2+pad_size*2:] = masks_pred_softmax[:, 3, :, :]
    
    images_batch[:, :, h*2+pad_size*2:, w*2+pad_size*2:] = masks_pred_softmax[:, 4, :, :][:, None, :, :]
